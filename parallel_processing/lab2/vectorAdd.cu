...
int main(int argc, char **argv)
{
  ...
  cudaMalloc((void **)&deviceInput1, sizeof(float) * inputLength1);
  cudaMalloc((void **)&deviceInput2, sizeof(float) * inputLength2);
  cudaMalloc((void **)&deviceOutput, sizeof(float) * inputLength1);
  ...
  cudaMemcpy(deviceInput1, hostInput1, sizeof(float) * inputLength1, cudaMemcpyHostToDevice);
  cudaMemcpy(deviceInput2, hostInput2, sizeof(float) * inputLength2, cudaMemcpyHostToDevice);

  cutStopTimer(gpu);
  printf("Copying input memory to the GPU: %f ms\n", cutGetTimerValue(gpu));

  //@@ Initialize the grid and block dimensions here
  // These have to be called gridDim and blockDim or else the code won't compile
  dim3 gridDim(ceil(inputLength1 / 256.0), 1, 1);
  dim3 blockDim(256, 1, 1);

  if (blog)
    printf("*** Block dimension is %i\n", blockDim.x);
  if (blog)
    printf("*** Grid dimension is %i\n", gridDim.x);
  ...
  cudaMemcpy(hostOutput, deviceOutput, sizeof(float) * inputLength1, cudaMemcpyDeviceToHost);
